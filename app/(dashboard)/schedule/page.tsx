...
// Colonne des jours + une colonne par heure, partagé par l'en-tête et les lignes
const GRID_COLS = "grid-cols-[110px_repeat(10,minmax(124px,1fr))]";
...
const decimalHour = (start: string) =>
    parseInt(start.split(":")[0]) + parseInt(start.split(":")[1] || "0") / 60;

type Band = { index: number; count: number };

// Répartit verticalement les cours d'une journée dans leur case.
// Les cours qui se chevauchent forment une grappe et reçoivent chacun une bande :
// [A] en haut, puis cours normaux et [GR], puis [B] en bas.
// Un cours seul prend toute la case, sauf [A]/[GR] (moitié haute) et [B] (moitié basse).
function assignBands(dayEntries: ScheduleEntry[]): Record<number, Band> {
    const overlaps = (a: ScheduleEntry, b: ScheduleEntry) => {
        const as = decimalHour(a.start), bs = decimalHour(b.start);
        return as < bs + b.duration && as + a.duration > bs;
    };
    const rank = (e: ScheduleEntry) => e.week === "A" ? 0 : e.week === "B" ? 2 : 1;
    ...
    for (const entry of dayEntries) {
        ...
        for (let i = 0; i < cluster.length; i++) {
            for (const other of dayEntries) {
                if (!seen.has(other.id) && overlaps(cluster[i], other)) {
                    seen.add(other.id);
                    cluster.push(other);
                }
            }
        ...
        if (cluster.length === 1) {
            bands[entry.id] =
                entry.week === "A" || entry.group ? { index: 0, count: 2 } :
                entry.week === "B" ? { index: 1, count: 2 } :
                { index: 0, count: 1 };
            continue;
        }
        ...
        cluster
            ...
            .forEach((e, index) => { bands[e.id] = { index, count: cluster.length }; });
    }
    return bands;
}
    ...
    return (
        <div className="space-y-4 h-[calc(100vh-8rem)] flex flex-col relative">
            ...
            <style jsx global>{`
                ...
                /* Pause de midi : hachures légères */
                .lunch-hatch {
            ...
            {/* Timetable Grid */}
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex-1 min-h-0 overflow-hidden flex flex-col relative">
                {/* Zone de défilement unique : la ligne des heures et la colonne des jours y restent fixes */}
                <div className="flex-1 min-h-0 overflow-auto custom-scrollbar">
                    <div className="min-w-[1350px] min-h-full flex flex-col print-grid-container">
                        {/* Ligne des heures */}
                        <div className={`grid ${GRID_COLS} sticky top-0 z-30 print:static border-b border-slate-300`}>
                            <div className="sticky left-0 z-40 print:static h-11 bg-slate-100 border-r border-slate-300 flex items-center justify-center text-slate-400">
                            ...
                            {HOURS.map((hour) => {
                                ...
                                return (
                                    <div
                                        key={hour}
                                        className={`h-11 flex flex-col items-center justify-center border-l border-slate-300 first:border-l-0 ${isLunch ? "bg-slate-200 lunch-hatch" : "bg-slate-100"}`}
                                    >
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {HOURS.map((hour, hi) => (
                                        <div
                                            key={hour}
                                            style={{ gridColumnStart: hi + 2 }}
                                            className={`row-start-1 border-l border-slate-200 ${hour === LUNCH_HOUR ? "bg-slate-50 lunch-hatch" : ""}`}
                                        />
                                    ))}

                                    {/* Cours de la journée */}
                                    <div className="col-[2/-1] row-start-1 relative">
                                        {dayEntries.map(item => {
                                            const offset = decimalHour(item.start) - FIRST_HOUR;
                                            const band = bands[item.id] ?? { index: 0, count: 1 };
                                            const isShared = band.count > 1;
                                            ...
                                            const badges = [
                                                ...
                                                item.week === "B" && { label: "B", className: "bg-orange-500 text-white" },
                                                item.group && { label: "GR", className: "bg-violet-600 text-white" },
                                            ].filter(Boolean) as { label: string; className: string }[];

                                            return (
                                                <div
                                                    key={item.id}
                                                    className="absolute z-10 flex flex-col group/slot pointer-events-none p-0.5"
                                                    style={{
                                                        left: `${(offset / HOURS.length) * 100}%`,
                                                        width: `${(item.duration / HOURS.length) * 100}%`,
                                                        top: `${(band.index / band.count) * 100}%`,
                                                        height: `${100 / band.count}%`,
                                                    }}
                                                >
                                                    <div
                ...
                {!selectedId && (
                    <div className="absolute inset-0 flex items-center justify-center bg-slate-50/50 backdrop-blur-[2px] z-50">
