...
import { Search, Plus, Filter, Send, CheckCircle2, Clock, Loader2, User, User2, GraduationCap, UserX, X, Save, CalendarDays, FileText, Trash2, BellRing, Check, ChevronDown, RefreshCw } from "lucide-react";
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                ...
                                <th className="p-4 text-xs font-semibold uppercase text-slate-500 tracking-wider">Créneau</th>
                                <th className="p-4 text-xs font-semibold uppercase text-slate-500 tracking-wider">Type</th>
                                {isAdmin && <th className="p-4 text-xs font-semibold uppercase text-slate-500 tracking-wider">Parent</th>}
                                <th className="p-4 text-xs font-semibold uppercase text-slate-500 tracking-wider text-right">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {filteredabsences.map((absence, index) => {
                                const estEnvoyee = absence.validated === true;
                                return (
                                    <motion.tr
                                        key={absence.id}
                                    ...
                                    >
                                        ...
                                        <td className="p-4">
                                            {absence.student ? (
                                                isAdmin ? (
                                                    <Link href={`/students?highlight=${absence.studentId}`} className="flex items-center gap-2 text-slate-600 hover:text-indigo-600 transition-colors">
                                                ...
                                                return (
                                                    <div className="relative inline-block">
                                                        <button
                                                            ...
                                                            disabled={changingId === absence.id}
                                                            className={`px-2.5 py-1 rounded-lg text-xs font-bold border inline-flex items-center gap-1.5 hover:brightness-95 transition-all disabled:opacity-50 ${style}`}
                                        ...
                                        {/* Statut d'envoi au parent — administration uniquement */}
                                                ...
                                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-200">
                                                    ...
                                                    {absence.validatedAt && (
                                                        <span className="font-medium opacity-70">
                                                            le {new Date(absence.validatedAt).toLocaleDateString("fr-FR")}
                                                        </span>
                                                    )}
                                                </span>
                                            ) : (
                                            ...
                                            <div className="flex items-center justify-end gap-2">
                                                {isAdmin && !estEnvoyee && (
                                                    <button
                                                        ...
                                                        className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-indigo-50 text-indigo-700 text-sm font-medium hover:bg-indigo-100 border border-indigo-200 transition-colors disabled:opacity-50"
